...
const translations = reactive({
  en: {
    ...
    wssReportTip: 'Enabling WSS reduces Workers requests and keeps realtime updates fast. Agent and frontend sockets use Durable Objects WebSocket Hibernation, so idle connections can sleep; when no visible frontend is watching, Agents fall back to their normal report interval to reduce DO duration.',
    ...
    userBackground: 'User background mode',
    userBackgroundTip: 'For Linux user sessions without a usable systemd --user manager (for example, some HPC/bastion environments). It survives logout but may not survive node reboot or administrator cleanup.',
  ...
  zh: {
    ...
    wssReportTip: '开启 WSS 可减少 Workers 请求量并保持低延迟实时刷新。Agent 与前端连接均使用 Durable Objects WebSocket Hibernation；无人可见地查看面板时，Agent 会回退到各自正常上报间隔，让 DO 在空闲期间休眠并降低时长消耗。',
    publicAccess: '公开访问',
    showPrice: '显示价格',
    showExpire: '显示到期时间',
    showTf: '显示流量配额',
    ...
    userBackground: '无 systemd 用户后台模式',
    userBackgroundTip: '仅用于无法使用 systemd --user 的 Linux 普通用户环境（如部分 HPC/堡垒机会话）。退出登录后可继续运行，但节点重启或管理员清理后台进程后不会自动恢复。',
    pingNodes: 'Ping节点',
    customCt: '电信测试节点',
    customCu: '联通测试节点',
    customCm: '移动测试节点',
